...
class Level:
    ...
    def load_map_data(self):
        file_name = self.num_of_level+'.json'
    ...
    def setup_ground_items(self):
        ...
        if self.num_of_level=='level_3' or self.num_of_level=='level_4':
            for name in ['ground', 'step']:
                for item in self.map_data[name]:
                    self.ground_items_group.add(stuff.Item(item['x'], item['y'], item['width'], item['height'], name))
        if self.num_of_level=='level_1' or self.num_of_level=='level_2': 
            ...
            for brick_data in self.map_data['brick']:
                ...
                if brick_type == 0:
                    if 'brick_num' in brick_data:
                        ...
                        self.brick_group.add(brick.Brick(x, y, brick_type,None, self, color=color))
                elif brick_type == 1:
                    self.brick_group.add(brick.Brick(x, y, brick_type, self.coin_group, self, color))
                else:
                    self.brick_group.add(brick.Brick(x, y, brick_type, self.powerup_group, self, color))
    ...
    def update_player_position(self):
        ...
        if self.player.rect.x < self.start_x:
            ...
            self.player.rect.right = self.end_x  # 不能走出画面
        if self.num_of_level=='level_1':
            if self.player.rect.centerx > end_x1:
                self.finished=True
                self.next='load_screen'
                self.num_of_level='level_2'
        elif self.num_of_level=='level_2':
            if self.player.rect.right>end_x2:
                self.finished=True
                self.next='load_screen'
                self.num_of_level='level_3'
        elif self.num_of_level=='level_3':
            if self.player.rect.right>end_x3:
                self.finished=True
                self.next='load_screen'
                self.num_of_level='level_4'
    ...
    def adjust_player_y(self, sprite):
        ...
        # upwards
            ...
            if sprite.name == 'brick':
                if (self.player.big or self.player.fire) and sprite.brick_type == 0:  # when mario is big and brick contains nothing
    ...
    def update_game_window(self):
        ...
        if self.player.x_vel > 0 and self.player.rect.centerx > third and self.game_window.right < self.end_x-5:
            self.game_window.x += self.player.x_vel
            self.start_x = self.game_window.x  # 不能走回头路

    def draw(self, surface):
        # blit(source, dest, area=None, special_flags=0) -> Rect
        # source:surface对象; dest:左上角坐标; area:Rect对象，限定surface对象的范围; 返回Rect对象;
