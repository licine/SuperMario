...
def create_enemy(enemy_data):
    ...
    enemy=enemy = Goomba(x, y_bottom, direction, "goomba", color)
...
class Enemy(pygame.sprite.Sprite):
    ...
    def load_frames(self, frame_rects):
        for frame_rect in frame_rects:
            ...
            shooted_frame=pygame.transform.rotate(left_frame,180)
            ...
            self.shooted_frames.append(shooted_frame)

    def update(self, level):
        self.curremt_time = pygame.time.get_ticks()
    ...
    def handle_states(self, level):
        ...
        if self.direction==1:
            ...
            self.image=self.shooted_frames[self.frame_index]

    def walk(self):
        if self.curremt_time - self.timer > 125:
            self.frame_index = (self.frame_index + 1) % 2
            self.image = self.frames[self.frame_index]
            self.timer = self.curremt_time
    ...
    def go_die(self, how, direction=1):
        self.death_timer = self.curremt_time
...
class Goomba(Enemy):
    ...
    def trampled(self, level):
        ...
        if self.death_timer == 0:
            self.death_timer = self.curremt_time
        if self.curremt_time - self.death_timer > 500:
            self.kill()
...
class Koopa(Enemy):
    def __init__(self, x, y_bottom, direction, name, color):
        ...
        dark_frame_rects = [(96, 41, 16, 22), (112, 41, 16, 22), (160, 41, 16, 22)]
    ...
    def trampled(self, level):
        ...
        if self.shell_timer == 0:
            self.shell_timer = self.curremt_time
        if self.curremt_time - self.shell_timer > 5000:
            self.state = 'walk'
