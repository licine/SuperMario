...
def create_powerup(centerx, centery, type, big):
...
class Powerup(pygame.sprite.Sprite):
    def __init__(self, centerx, centery, frame_rects):
        ...
        for frame_rect in frame_rects:
            self.frames.append(tools.get_image(setup.GRAPHICS['item_objects'], *frame_rect,
                                               (0, 0, 0), 2.5))
    ...
    def check_y_collisions(self, level):
        ...
        if sprite:
            if self.rect.top < sprite.rect.top:
                self.rect.bottom = sprite.rect.top
                self.y_vel = -10
        shooted_enemy = pygame.sprite.spritecollideany(self, level.enemy_group)
        if shooted_enemy:
            if level.player.rect.centerx > shooted_enemy.rect.centerx:
                shooted_enemy.go_die('bumped',-1)
            else:
                shooted_enemy.go_die('bumped')
            level.enemy_group.remove(shooted_enemy)
            level.dying_group.add(shooted_enemy)
            self.state='boom'
