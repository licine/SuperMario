...
class Brick(pygame.sprite.Sprite):
    def __init__(self, x, y, brick_type, group, level, color=None, name='brick'):
        ...
        for frame_rect in self.frame_rects:
            self.frames.append(tools.get_image(setup.GRAPHICS['tile_set']
                                               , *frame_rect, (0, 0, 0), C.BRICK_MULTI))
        self.frame_index = 0
    ...
    def bumped(self):
        ...
        if self.rect.y > self.y + 5:
            ...
            if self.brick_type == 0:
                ...
                self.group.add(create_powerup(self.rect.centerx, self.rect.centery, self.brick_type, self.level.player.big))
                self.state='rest'
